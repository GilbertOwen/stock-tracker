    ...
    <div class="bg-white rounded-xl border border-ink-300 overflow-hidden mb-4">
        ...
        <form action="{{ route('barangs.update', $barang->id_barang) }}" method="POST" class="px-6 py-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="harga_beli" class="block text-xs font-semibold text-ink-700 mb-1.5 uppercase tracking-wide">
                        Harga Beli
                        <span class="normal-case text-ink-400 font-normal ml-1">(opsional)</span>
                    </label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-4 flex items-center text-sm font-semibold text-ink-400 pointer-events-none">Rp</span>
                        <input type="number" id="harga_beli" name="harga_beli" value="{{ old('harga_beli', $barang->harga_beli) }}" placeholder="0" min="0"
                            class="w-full pl-10 pr-4 py-2.5 rounded-lg border text-sm font-medium text-ink-900 placeholder-ink-300
                                   focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-brand-400 transition
                                   {{ $errors->has('harga_beli') ? 'border-red-400 bg-red-50' : 'border-ink-300 bg-white hover:border-ink-400' }}"
                        />
                    </div>
                    @error('harga_beli')
                ...
                <div>
                    <label for="harga_jual" class="block text-xs font-semibold text-ink-700 mb-1.5 uppercase tracking-wide">
                        Harga Jual
                        <span class="normal-case text-ink-400 font-normal ml-1">(opsional)</span>
                    </label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-4 flex items-center text-sm font-semibold text-ink-400 pointer-events-none">Rp</span>
                        <input type="number" id="harga_jual" name="harga_jual" value="{{ old('harga_jual', $barang->harga_jual) }}" placeholder="0" min="0"
                            class="w-full pl-10 pr-4 py-2.5 rounded-lg border text-sm font-medium text-ink-900 placeholder-ink-300
                                   focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-brand-400 transition
                                   {{ $errors->has('harga_jual') ? 'border-red-400 bg-red-50' : 'border-ink-300 bg-white hover:border-ink-400' }}"
                        />
                    </div>
                    @error('harga_jual')
                        <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
